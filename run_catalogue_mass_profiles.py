...
def density_marginalized_radius_probability(
    radius_um,
    conditional_delta_chi2,
    *,
    reference_density_kg_m3=METEOROID_DENSITY_KG_M3,
    density_bounds_kg_m3=(100.0, 8000.0),
    density_grid_size=257,
):
    """Marginalize a fixed-density radius profile over meteoroid density.

    For the spherical Ceplecha model, ``rho_m * r`` is the dynamically
    identifiable quantity: scaling radius by k and density by 1/k leaves both
    drag and fractional ablation unchanged.  The input profile is therefore a
    likelihood in equivalent radius at ``reference_density_kg_m3``.

    A log-uniform density prior is used over the supplied bounds.  The returned
    joint weights are useful for propagating the same uncertainty into mass.
    """
    radius_um = np.asarray(radius_um, dtype=np.float64)
    conditional_delta_chi2 = np.asarray(conditional_delta_chi2, dtype=np.float64)
    if radius_um.ndim != 1 or conditional_delta_chi2.shape != radius_um.shape:
        raise ValueError("radius and conditional chi-square arrays must be one-dimensional and equal length")
    if len(radius_um) < 2 or np.any(radius_um <= 0.0) or np.any(np.diff(radius_um) <= 0.0):
        raise ValueError("radius grid must contain at least two strictly increasing positive values")
    density_min, density_max = map(float, density_bounds_kg_m3)
    if not (0.0 < density_min < density_max):
        raise ValueError("density bounds must be positive and increasing")

    density_kg_m3 = np.geomspace(density_min, density_max, int(density_grid_size))
    equivalent_radius_um = (
        radius_um[:, None] * density_kg_m3[None, :] / float(reference_density_kg_m3)
    )
    log_radius = np.log(radius_um)
    conditional = np.interp(
        np.log(equivalent_radius_um),
        log_radius,
        conditional_delta_chi2,
        left=np.inf,
        right=np.inf,
    ...
    joint_likelihood = np.zeros_like(conditional)
    if np.any(finite):
        minimum = float(np.min(conditional[finite]))
        joint_likelihood[finite] = np.exp(
            -0.5 * np.clip(conditional[finite] - minimum, 0.0, 700.0)
        )

    log_edges = np.empty(len(log_radius) + 1, dtype=np.float64)
    log_edges[1:-1] = 0.5 * (log_radius[:-1] + log_radius[1:])
    log_edges[0] = log_radius[0] - 0.5 * (log_radius[1] - log_radius[0])
    log_edges[-1] = log_radius[-1] + 0.5 * (log_radius[-1] - log_radius[-2])
    radius_bin_width = np.diff(log_edges)
    density_bin_width = (np.log(density_max) - np.log(density_min)) / (len(density_kg_m3) - 1)
    joint_weights = joint_likelihood * radius_bin_width[:, None] * density_bin_width
    total = float(np.sum(joint_weights))
    if total > 0.0:
        joint_weights /= total
    radius_weights = np.sum(joint_weights, axis=1)
    probability_density = np.divide(
        radius_weights,
        radius_bin_width,
        out=np.zeros_like(radius_weights),
        where=radius_bin_width > 0.0,
    )
    return probability_density, radius_weights, density_kg_m3, joint_weights, conditional
