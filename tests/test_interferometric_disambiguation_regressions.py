...
@pytest.mark.slow
def test_event_540174_correct_hypothesis_has_good_range_fit(tmp_path):
    cut_dir = PANSY_RECEIVER_ROOT / "data" / "metadata" / "cut"
    if not cut_dir.exists():
        pytest.skip(f"local cut metadata not available: {cut_dir}")

    output_dir = tmp_path / "event_540174"
    cmd = [
        sys.executable,
        str(PANSY_RECEIVER_ROOT / "plot_interferometric_disambiguation.py"),
        "--sample-idx",
        str(RANGE_FIT_SAMPLE_540174),
        "--cut-dir",
        str(cut_dir),
        "--output-dir",
        str(output_dir),
    ...
    diagnostics_h5 = output_dir / f"pansy_disambiguation_diagnostics_{RANGE_FIT_SAMPLE_540174}.h5"
    assert diagnostics_h5.exists(), result.stdout
